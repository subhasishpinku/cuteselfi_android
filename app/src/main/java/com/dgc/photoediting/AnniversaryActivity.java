...
public class AnniversaryActivity extends AppCompatActivity {
    Toolbar toolbar;
    private List<String> listImagesAnniversary = new ArrayList<>();
    GridView grid;
    @Override
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                switch (position) {
                    default:
                        ...
                        intent.putExtra("image", listImagesAnniversary.get(position));
                        intent.putExtra("title","Anniversary");
    ...
    public void getfetchFrames(){
        if (Utility.checkConnectivity(getApplicationContext())) {
                            ...
                            for (int i = 0; i <response.body().getPayloadFetchFrames().size(); i++) {
                                ...
                                if (label.equals("Anniversary")) {
                                    for (int j = 0; j < getPayloadFetchFrames.getFramesIds().size(); j++) {
                                        listImagesAnniversary.add(getPayloadFetchFrames.getFramesIds().get(j));
                                    }
                                }
                            }
                            MyGridAdapter adapter = new MyGridAdapter(getApplicationContext(), listImagesAnniversary);
            ...
            PopupClass.showPopUpWithTitleMessageOneButton(AnniversaryActivity.this, "Ok", "", "Sorry, No Internet connection found, Please check your network connection", "", new PopupCallBackOneButton() {
                @Override
